###############################################
### Source repository: https://github.com/bilby-dev/bilby
### Original file path: bilby/bilby/gw/source.py
###############################################
import numpy as np
...
IMRPhenomD_gen_IMRPhenomD_polar_jit = jit(IMRPhenomD.gen_IMRPhenomD_polar, backend='cpu')

def ripple_binary_black_hole(frequency_array, 
    ...
    lambda_sigma_30,
    lambda_sigma_31,
    lambda_sigma_32, **kwargs):
    """
    Tested for:
        reference_frequency = 10.0
        minimum_frequency = {H1:10.00, L1:10.00, V1:10.00, waveform:10.00}
    ...
    frequency_array_ripple = jnp.array(frequency_array[(frequency_array>=f_min) & (frequency_array<=f_max)])
    
    tc = 0.0 # Time of coalescence in seconds set to zero because it is handled by the likelihood
    polarization_angle = 0.0 # Polarization angle set to zero because it is handled by the likelihood

    lambda_sigma = jnp.array([lambda_sigma_0, lambda_sigma_1, lambda_sigma_2,
                          lambda_sigma_3, lambda_sigma_4, lambda_sigma_5,
                          lambda_sigma_6, lambda_sigma_7, lambda_sigma_8,
                          lambda_sigma_9, lambda_sigma_10, lambda_sigma_11,
                          lambda_sigma_12, lambda_sigma_13, lambda_sigma_14,
                          lambda_sigma_15, lambda_sigma_16, lambda_sigma_17,
                          lambda_sigma_18, lambda_sigma_19, lambda_sigma_20,
                          lambda_sigma_21, lambda_sigma_22, lambda_sigma_23,
                          lambda_sigma_24, lambda_sigma_25, lambda_sigma_26,
                          lambda_sigma_27, lambda_sigma_28, lambda_sigma_29,
                          lambda_sigma_30, lambda_sigma_31, lambda_sigma_32])
    ...
    theta_ripple = jnp.array([Mc, eta, chi_1, chi_2, luminosity_distance, tc, phase, theta_jn, polarization_angle])
    hp_ripple_modified, hc_ripple_modified = IMRPhenomD_gen_IMRPhenomD_polar_jit(frequency_array_ripple, theta_ripple, f_ref, lambda_sigma)
    
    hp_ripple_modified_padded = np.zeros_like(frequency_array, dtype=np.complex128)
    hc_ripple_modified_padded = np.zeros_like(frequency_array, dtype=np.complex128)

    hp_ripple_modified_padded[-len(hp_ripple_modified):] = hp_ripple_modified
    hc_ripple_modified_padded[-len(hc_ripple_modified):] = hc_ripple_modified

    return {'plus': hp_ripple_modified_padded, 'cross': hc_ripple_modified_padded}

IMRPhenomD_gen_IMRPhenomD_polar_ppe_jit = jit(IMRPhenomD.gen_IMRPhenomD_polar_ppe, backend='cpu')

def ripple_binary_black_hole_ppe(frequency_array, 
    mass_1, mass_2, chi_1, chi_2, 
    luminosity_distance, theta_jn, phase,
    lambda_sigma_0,
    lambda_sigma_1,
    ...
    theta_ripple = jnp.array([Mc, eta, chi_1, chi_2, luminosity_distance, tc, phase, theta_jn, polarization_angle])
    hp_ripple_modified, hc_ripple_modified = IMRPhenomD_gen_IMRPhenomD_polar_ppe_jit(frequency_array_ripple, theta_ripple, f_ref, lambda_sigma, beta_ppe, b_ppe)
...
def _base_lal_cbc_fd_waveform(
    ...
    if ('mode_array' in waveform_kwargs) and waveform_kwargs['mode_array'] is not None:
        ...
        for mode in mode_array:
            mode_0_int = int(mode[0])
            mode_1_int = int(mode[1])
            lalsim.SimInspiralModeArrayActivateMode(mode_array_lal, mode_0_int, mode_1_int)
